...
Die GERIT-DESTATIS-Datei entsteht aus GERIT-Daten und DESTATIS-Daten. Im
Repo liegt dafuer `merge_gerit_with_DESTATIS_system.R`. Die Quelldaten
kommen aus [GERIT](https://www.gerit.org/de/) und
...
Spaltennamen koennen ueber `organisation_col` angegeben werden. Der
Workflow wandelt Spaltennamen intern in Kleinbuchstaben um.

Wichtig: Der Workflow matched nicht jede Kurszeile einzeln. Er
extrahiert zuerst eindeutige Organisationen, matched diese eindeutigen
Werte einmal und spielt das Ergebnis danach wieder auf die
vollstaendigen gescrapten Daten zurueck. Das spart API-Aufrufe und sorgt
dafuer, dass gleiche Organisationsnamen konsistent gleich behandelt
werden.
...
`USER` und `LOGNAME` geprueft. Der Name landet spaeter in Dateinamen und
Metadaten.

`df_scraped` wird in ein Tibble umgewandelt. Die eigentliche
...
- Falls noetig, werden alte bzw. alternative Spaltennamen
  vereinheitlicht: `Hochschul_Name` wird zu `HS`, `Gerit_Orga` wird zu
  `Einrichtung`.
- Fehlende optionale Spalten werden mit `NA` angelegt, zum Beispiel
  `Fachgebiet_Gerit_1` bis `Fachgebiet_Gerit_6`, `Faechergruppen_IDs`
  und `LUF_Namen`.
- Pflichtspalten werden geprueft: `HS` und `Einrichtung`.
- `name_gerit` muss exakt in `HS` vorkommen. Wenn nicht, gibt die
  Funktion alle erlaubten Hochschulnamen aus und bricht ab.
...
- Eine `gerit_ID` wird erzeugt oder vorhandene IDs werden als Integer
  aufbereitet.
- `gerit_cleaned` wird als normalisierte Variante von `Einrichtung`
  erzeugt.
- `unique_name_for_einrichtung` markiert, ob ein Einrichtungsname
  innerhalb der Hochschule eindeutig ist.
...
eindeutiger Organisations-Ebene.

Dabei passiert Folgendes:

- Spaltennamen werden kleingeschrieben.
- Die konfigurierte Organisationsspalte wird intern `organisation`
  genannt.
...
Die Normalisierung in `normalize_text()` ist bewusst einfach und
transparent:
...
- haeufige Strukturbegriffe wie `Institut`, `Professur`, `Department`,
  ...
  `Arbeitsgruppe`, `Zentrum`, `Arbeitsstelle` und `Chair` werden
  entfernt.
- `fur` wird entfernt.
...
- `direct`: Der rohe Organisationsname entspricht exakt einem
  eindeutigen GERIT-`Einrichtung`-Wert.
...
automatisch uebernommen. Wenn derselbe GERIT-Name mehrfach vorkommt,
wird er nicht deterministisch gematcht. Dadurch werden schnelle, aber
riskante Mehrdeutigkeiten vermieden.

Bei einem deterministischen Treffer werden die GERIT-Informationen
direkt in die Match-Spalten geschrieben:
...
Dieser Schritt erzeugt noch keine finale Entscheidung. Er erzeugt nur
eine gerankte Kandidatenliste fuer das LLM.

So funktioniert es:

- Als GERIT-Kandidaten werden nur Eintraege genutzt, deren
  `unique_name_for_einrichtung == "ja"` ist.
- Fuer jeden offenen Scraping-Wert wird ein Embedding-Text gebaut. In
  der Praxis ist das der rohe Organisationsname, mit Fallback auf
  `cleaned`.
- Fuer jeden GERIT-Kandidaten wird der Einrichtungsname als
  Embedding-Text genutzt.
...
Ranking-Signal. Er ist keine kalibrierte Wahrscheinlichkeit und nicht
die finale Match-Entscheidung.
...
Der Cache speichert Kombinationen aus Modellname, Eingabetext und
Embedding. Bereits vorhandene GERIT-Embeddings werden wiederverwendet.
...
Bei `select_candidate` muss `selected_candidate_id` exakt eine der
erlaubten `gerit_ID`s aus der Kandidatenliste sein. Jede andere ID ist
...
Wenn das LLM `select_candidate` waehlt und die ID gueltig ist, werden
die GERIT-Daten ueber `lookup_match_from_gerit()` in die Match-Spalten
uebernommen:
...
Wenn das LLM `no_match` waehlt, erzeugt `not_match_record()` einen
leeren Match-Datensatz:
...
Wenn `auto_review = TRUE` und mindestens ein solcher Fall existiert,
startet
...
Die Zeile wird intern wieder in Komponenten zerlegt, jede Komponente
wird in der Organisationstabelle nachgeschlagen und die Ergebnisse
werden danach auf die urspruengliche Zeile aggregiert.

Die Aggregationslogik ist:

- `matched = "yes"`, wenn alle Komponenten der Zeile gematcht sind
- `matched = "partial"`, wenn mindestens eine, aber nicht alle
  Komponenten gematcht sind
...
`gold_data` kann ein Data Frame oder ein Pfad zu einer `.rds`-Datei
sein. Die Spaltennamen werden in Kleinbuchstaben umgewandelt. Erwartet
werden:
...
`LUF_IDs` aus GERIT werden mit `gold_data$luf_code` verglichen.
...
Direkt danach ruft der Workflow
...
- Er laesst das LLM nicht frei halluzinieren. Das LLM darf nur
  Kandidaten aus der vorgelegten Liste waehlen oder `no_match` sagen.
- Er uebernimmt deterministische Matches nur, wenn die GERIT-Seite
  eindeutig ist.
...
- Er speichert den wichtigsten Output frueh als `.rds`, damit das
...
- [`find_names()`](https://github.com/Stifterverband/HEXmatchR/reference/find_names.md)
  listet gueltige Hochschulnamen aus der GERIT-Datei.
