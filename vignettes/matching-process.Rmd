...
<img src="figures/gerit_destatis_match.png" alt="GERIT DESTATIS Match" width="400"/>


## Das Matching - Der Prozess Schritt für Schritt

Der Prozess des Matchings wurde so einfach wie möglich und so komplex wie nötig gestaltet. Im folgenden wird der detaillierte Ablauf des Matching-Workflows beschrieben, wie er in der Funktion  [`run_matching_workflow()`](https://stifterverband.github.io/HEXmatchR/reference/run_matching_workflow.html) implementiert ist.
...
Der Workflow ermittelt zuerst den aktuellen Nutzernamen über `current_username()`. Dieser wird verwendet, um zu dokumentieren, wer den Matching-Workflow ausgeführt hat und, falls nötig, wer mögliche Review-Fälle bearbeitet hat.

### 2. GERIT-DESTATIS-Daten für eine Hochschule laden

`prepare_gerit_data(name_gerit)` liest standardmäßig:
...
-   Falls nötig, werden alte bzw. alternative Spaltennamen vereinheitlicht: `Hochschul_Name` wird zu `HS`, `Gerit_Orga` wird zu `Einrichtung`.
...
-   Pflichtspalten werden geprüft: `HS` und `Einrichtung`.
...
Diese gefilterte GERIT-Tabelle ist die Referenzmenge für alle folgenden Schritte.
...
-   Leere oder fehlende Organisationsstücke werden entfernt.
-   Identische Organisationsnamen werden dedupliziert.
-   Für jede Organisation wird gespeichert, aus welchen ursprünglichen Organisationsstrings sie kam. Diese Rückverknüpfung steht in `organisation_names_for_matching_back`.
-   `organisation_original` enthält den Originalwert.
-   `cleaned` enthält eine normalisierte Textfassung.
...
-   `&` wird zu `und`.
-   häufige Strukturbegriffe wie `Institut`, `Professur`, `Department`, `Fachbereich`, `Seminar`, `Klinik`, `Abteilung`, `Lehrstuhl`, `Arbeitsgruppe`, `Zentrum`, `Arbeitsstelle` und `Chair` werden entfernt.
-   `fur` wird entfernt.
...
``` text
Institut für Wirtschaftsinformatik (Lehrstuhl A)
```
...
Wichtig für die Transparenz: Es werden nur eindeutige GERIT-Treffer automatisch übernommen. Wenn derselbe GERIT-Name mehrfach vorkommt, wird er nicht deterministisch gematcht. Dadurch werden schnelle, aber riskante Mehrdeutigkeiten vermieden.
...
Dieser Schritt erzeugt noch keine finale Entscheidung. Er erzeugt nur eine gerankte Kandidatenliste für das LLM.

So funktioniert es:

-   Als GERIT-Kandidaten werden nur Einträge genutzt, deren `unique_name_for_einrichtung == "ja"` ist.
-   Für jeden offenen Scraping-Wert wird ein Embedding-Text gebaut. In der Praxis ist das der rohe Organisationsname, mit Fallback auf `cleaned`.
-   Für jeden GERIT-Kandidaten wird der Einrichtungsname als Embedding-Text genutzt.
-   Beide Textmengen werden mit dem OpenAI Embedding-Modell eingebettet, standardmäßig `text-embedding-3-large`.
-   Für jede offene Organisation wird die Kosinusähnlichkeit zu allen GERIT-Kandidaten berechnet.
-   Die besten `top_k` Kandidaten werden behalten, standardmäßig 5.
...
Der `score` ist eine Kosinusähnlichkeit der Embeddings. Er ist nur ein Ranking-Signal. Er ist keine kalibrierte Wahrscheinlichkeit und nicht die finale Match-Entscheidung.

GERIT-Embeddings werden standardmäßig lokal gecacht:

``` text
data/cache/gerit_embeddings_<embedding_model>.rds
```

Der Cache speichert Kombinationen aus Modellname, Eingabetext und Embedding. Bereits vorhandene GERIT-Embeddings werden wiederverwendet. Query-Embeddings der gescrapten Organisationen werden absichtlich nicht dauerhaft gecacht, weil sie laufabhängig sind.
...
Für jede offene Organisation sieht das LLM:
...
Bei `select_candidate` muss `selected_candidate_id` exakt eine der erlaubten `gerit_ID`s aus der Kandidatenliste sein. Jede andere ID ist ungültig und wird als Review-Fall behandelt.
...
Die Pipeline nutzt standardmäßig:

-   Chat-Modell: `gpt-4.1-mini`
-   Temperatur: `0`
-   Review-Schwelle: `review_confidence = 0.65`

Die Konfidenz ist die Selbsteinschätzung des Modells zwischen 0 und 1. Sie wird so interpretiert:
...
Wenn das LLM `select_candidate` wählt und die ID gültig ist, werden die GERIT-Daten über `lookup_match_from_gerit()` in die Match-Spalten übernommen:
...
Wenn das LLM `no_match` wählt, erzeugt `not_match_record()` einen leeren Match-Datensatz:

-   alle GERIT-Felder werden `NA`
-   `matched = "no"`
-   `match_type = "review"` wenn Review nötig ist
-   sonst `match_type = "not_matchable"`
-   `match_confidence = NA`
-   `match_reason = reason`

Wenn für eine Organisation keine Embedding-Kandidaten erzeugt wurden, wird automatisch `no_match` mit Review gesetzt.
...
`df_scraped_matched` ist bereits auf die ursprünglichen Scraping-Zeilen zurückgespielt, aber noch vor einem optionalen manuellen Review.

### 8. Optionaler manueller Review

[`run_matching_workflow()`](https://github.com/Stifterverband/HEXmatchR/reference/run_matching_workflow.md) zählt Review-Fälle so:
...
-   vorhandenen Modell-Match bestätigen
-   einen anderen Kandidaten wählen
-   als kein Match markieren

Die App blockiert den Workflow, bis der Review abgeschlossen ist. Danach schreibt [`apply_review_decisions()`](https://github.com/Stifterverband/HEXmatchR/reference/apply_review_decisions.md) die manuellen Entscheidungen zurück in die Organisationsebene.

Wenn `auto_review = FALSE`, werden offene Review-Fälle nicht aufgelöst. Sie bleiben im Output sichtbar.

### 9. Zurückspielen auf die ursprünglichen gescrapten Daten

`join_matches_back_to_scraped()` führt die Organisationsebene wieder mit den ursprünglichen Kursdaten zusammen.
...
Die Zeile wird intern wieder in Komponenten zerlegt, jede Komponente wird in der Organisationstabelle nachgeschlagen und die Ergebnisse werden danach auf die ursprüngliche Zeile aggregiert.
...
-   Mehrfachwerte wie `LUF_IDs`, `Faechergruppen_IDs`, `match_type` und `match_reason` werden eindeutig kollabiert und mit `|` bzw. passenden Trennzeichen zusammengeführt
...
Wenn `gold_data` übergeben wird, startet [`evaluate_against_goldstandard()`](https://github.com/Stifterverband/HEXmatchR/reference/evaluate_against_goldstandard.md).
...
-   Organisationsspalte, standardmäßig `organisation`
-   `matchingart`
...
Der Workflow gibt eine Liste zurück:
...
Die vollständigen ursprünglichen gescrapten Daten mit angespielten Matching-Spalten. Das ist der zentrale Arbeitsoutput.

### `matched_organisations`

Alle Organisationen, die nach automatischem Matching und optionalem Review `matched == "yes"` haben. Zusätzlich werden Metadaten gesetzt:
...
Eine kompakte Tabelle der offenen oder unsicheren Fälle:
...
`NULL`, wenn kein `gold_data` übergeben wurde. Sonst eine Liste mit `comparison`, `metrics` und `by_gold_matchingart`.
...
Nur vorhanden, wenn `include_debug = TRUE`. Dann enthält der Output zusätzlich:
...
Für Fehlersuche, Audits und Nachvollziehbarkeit ist `include_debug = TRUE` sehr hilfreich.

## Bedeutung der wichtigsten Match-Spalten

| Spalte | Bedeutung |
|----|----|
| `gerit_ID` | Interne ID des ausgewählten GERIT-Eintrags |
...
| `Faechergruppen` | Fächergruppe(n) aus GERIT/DESTATIS |
| `Faechergruppen_IDs` | IDs der Fächergruppen |
...
| `match_reason` | Begründung des deterministischen, LLM- oder Review-Entscheids |
| `needs_review` | `TRUE`, wenn der Fall manuell geprüft werden sollte oder noch offen ist |
...
-   Er matched nicht blind auf den höchsten Embedding-Score. Der Score erzeugt nur Kandidaten.
-   Er lässt das LLM nicht frei halluzinieren. Das LLM darf nur Kandidaten aus der vorgelegten Liste wählen oder `no_match` sagen.
-   Er übernimmt deterministische Matches nur, wenn die GERIT-Seite eindeutig ist.
-   Er versteckt Unsicherheit nicht: Niedrige LLM-Konfidenz, fehlende Kandidaten und offene Nicht-Matches werden als Review-Fälle sichtbar.
-   Er speichert den wichtigsten Output früh als `.rds`, damit das Ergebnis auch nach späteren Fehlern im optionalen Goldstandard-Schritt verfügbar bleibt.
...
result <- run_matching_workflow(
  name_gerit = "Universität Beispielstadt",
...
Wenn keine Shiny-App geöffnet werden soll:
...
-   [`find_names()`](https://github.com/Stifterverband/HEXmatchR/reference/find_names.md) listet gültige Hochschulnamen aus der GERIT-Datei.
-   [`generate_candidate_matches()`](https://github.com/Stifterverband/HEXmatchR/reference/generate_candidate_matches.md) ist ein Komfort-Wrapper für [`generate_embedding_candidates()`](https://github.com/Stifterverband/HEXmatchR/reference/generate_embedding_candidates.md).
-   [`finalise_matching()`](https://github.com/Stifterverband/HEXmatchR/reference/finalise_matching.md) speichert Matching- und Review-Tabellen, wenn bestehende Organisationstabellen separat abgeschlossen werden sollen.
-   [`import_matching_data()`](https://github.com/Stifterverband/HEXmatchR/reference/import_matching_data.md) spielt bereits gespeicherte Matching-Ergebnisse nachträglich in Kurs-Rohdaten ein.
...
Die GERIT- und DESTATIS-Daten werden durch `merge_gerit_with_DESTATIS_system.R` zusammengeführt.
